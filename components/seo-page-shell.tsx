        ...
        <div className="mx-auto grid w-full max-w-7xl gap-10 px-5 py-20 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:px-8">
          <div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              ...
              <Link
                href="/templates"
                className="inline-flex items-center justify-center border border-white/20 bg-white/[0.06] px-6 py-3 text-sm font-bold text-white transition hover:border-white/35 hover:bg-white/[0.1]"
              >
        ...
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 overflow-hidden border border-border">
            <div className="grid grid-cols-[0.8fr_1fr_1fr] border-b border-border bg-background text-xs font-bold uppercase tracking-wide text-muted">
              ...
              <div className="p-4 text-success">OpenReply</div>
            ...
            {config.comparisons.map((item) => (
              ...
              >
                <div className="bg-background p-4 text-sm font-semibold text-foreground">
                  {item.label}
                </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {config.templateLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="border border-border bg-surface p-5 text-sm font-semibold text-foreground transition hover:border-border-hover hover:bg-surface-hover"
            >
              {link.label}
            </Link>
          ))}
      ...
      <section className="border-t border-white/10 bg-zinc-950/70 py-16">
        <div className="mx-auto grid w-full max-w-7xl gap-8 px-5 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:px-8">
          ...
          <div className="grid gap-3">
            {config.faqs.map((faq) => (
              <article key={faq.title} className="border border-white/15 bg-white/[0.06] p-5">
                <h3 className="text-lg font-bold text-white">{faq.title}</h3>
                <p className="mt-2 text-sm leading-6 text-zinc-300">{faq.body}</p>
              </article>
            ))}
          </div>
        </div>
